...
import { Appointment, AppointmentStage, EarningWindow, STAGE_LABELS, View, User, TeamMember, PayCycle, AvatarId, ActivityLog, Incentive, IncentiveRule, STAGE_COLORS } from './types';
...
import { IconMenu, IconMoon, IconPlus, IconSearch, IconSun, getAvatarIcon, IconTrash, IconTrophy, IconSparkles, IconTransfer, IconActivity, IconX } from './components/Icons';
...
export default function App() {
  ...
  const refreshData = useCallback(async () => {
    try {
        const [{ data: users }, { data: appointments }, { data: cycles }, { data: logs }, { data: incentives }, { data: rules }, { data: settings }] = await Promise.all([
            supabase.from('users').select('*'), supabase.from('appointments').select('*'), supabase.from('pay_cycles').select('*'),
            supabase.from('activity_logs').select('*'), supabase.from('incentives').select('*'), supabase.from('incentive_rules').select('*'),
  ...
  }, [allAppointments, activeCycle]);

  const handleUpdateMasterCommissions = async (standard: number, self: number, syncRetroactive: boolean = false) => {
    if (!isAdmin) return;
    const { error } = await supabase.from('settings').upsert({ id: 'global', commission_standard: standard, commission_self: self, updated_at: new Date().toISOString() });
    if (error) { alert(`Sync Error: ${error.message}`); return; }

    if (syncRetroactive && activeCycle) {
        const s = new Date(activeCycle.startDate).getTime();
        const e = new Date(activeCycle.endDate).setHours(23, 59, 59, 999);
        const cycleDeals = allAppointments.filter(a => a.stage === AppointmentStage.ONBOARDED && new Date(a.scheduledAt).getTime() >= s && new Date(a.scheduledAt).getTime() <= e);
        
        for (const appt of cycleDeals) {
            const agent = allUsers.find(u => u.id === appt.userId);
            const isSelf = (appt.aeName === agent?.name);
            const correctRate = isSelf ? self : standard;
            if (appt.earnedAmount !== correctRate) {
                await supabase.from('appointments').update({ earned_amount: correctRate }).eq('id', appt.id);
            }
        }
    }
    setCommissionRate(standard); setSelfCommissionRate(self); refreshData();
  };
  ...
  const handleSaveAppointment = async (data: any) => {
    ...
    const dbData = { name: data.name, phone: data.phone, email: data.email, scheduled_at: data.scheduledAt, stage: data.stage, notes: data.notes, user_id: finalUserId, type: data.type || 'appointment', ae_name: data.aeName, earned_amount: data.stage === AppointmentStage.ONBOARDED ? (baseRate + bonus) : 0 };
  ...
  const handleMoveStage = async (id: string, stage: AppointmentStage) => {
    ...
    if (stage === AppointmentStage.ONBOARDED) {
        if (appt.stage === AppointmentStage.TRANSFERRED) {
            ...
            refreshData();
        } else { setPendingMove({ id, stage: AppointmentStage.TRANSFERRED }); setIsAEModalOpen(true); }
        return;
    }
  ...
  const displayEarnings = useMemo(() => {
    ...
    const windows: EarningWindow[] = [...uniqueCycles].sort((a: PayCycle, b: PayCycle) => new Date(b.endDate).getTime() - new Date(a.endDate).getTime()).reduce<EarningWindow[]>((acc: EarningWindow[], cycle: PayCycle) => {
        ...
        acc.push({ id: cycle.id, userId: isAdminMode ? 'team' : user.id, startDate: cycle.startDate, endDate: cycle.endDate, totalCents: cycleAppts.reduce((s, a) => s + (a.earnedAmount || 0), 0) + cycleIncentives.reduce((s, i) => s + i.amountCents, 0), onboardedCount: cycleAppts.length, isClosed: now > end, incentives: cycleIncentives });
        return acc;
    }, []);
  ...
  return (
    <div className="h-screen flex bg-slate-50 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0" onDragOver={e => e.preventDefault()}>
        {isTickerVisible && <GlobalWinTicker appointments={allAppointments} users={allUsers} />}
           ...
           <div className="flex-1 max-w-xl mx-4">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-slate-100 dark:bg-slate-800 rounded-full py-2.5 pl-10 pr-10 text-sm border-none dark:text-white focus:ring-2 focus:ring-indigo-500/20 transition-all" 
                />
                {searchQuery && (
                  <button 
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar relative">
          {loadingAuth ? <div className="h-full flex items-center justify-center"><div className="w-10 h-10 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin"></div></div> : currentView === 'profile' ? <ProfileView user={user} onUpdateUser={(n, a, ns, pd) => supabase.from('users').update({name: n, avatar_id: a, notification_settings: ns, preferred_dialer: pd}).eq('id', user.id).then(() => refreshData())} totalEarnings={displayEarnings.lifetime} totalOnboarded={allAppointments.filter(a => (isAdmin ? true : a.userId === user.id) && a.stage === AppointmentStage.ONBOARDED).length} showFailedSection={showFailedSection} onToggleFailedSection={setShowFailedSection} onReplayTutorial={() => setForceTutorial(true)} /> : currentView === 'admin-dashboard' ? <AdminDashboard members={allUsers.filter(u => u.role !== 'admin').map(u => ({ id: u.id, name: u.name, role: u.role as any, status: 'Online', onboardedCount: allAppointments.filter(a => a.userId === u.id && a.stage === AppointmentStage.ONBOARDED).length, totalEarnings: (allAppointments.filter(a => a.userId === u.id && a.stage === AppointmentStage.ONBOARDED).reduce((s, a) => s + (a.earnedAmount || 0), 0)) + allIncentives.filter(i => i.userId === u.id || i.userId === 'team').reduce((s, i) => s + i.amountCents, 0), lastActive: 'Recently', avatarId: u.avatarId }))} payCycles={payCycles} onAddCycle={(s,e) => supabase.from('pay_cycles').insert({id: generateId(), start_date: s, end_date: e, status: 'active'}).then(() => refreshData())} onEditCycle={(id, s, e) => supabase.from('pay_cycles').update({start_date: s, end_date: e}).eq('id', id).then(() => refreshData())} onDeleteCycle={id => supabase.from('pay_cycles').delete().eq('id', id).then(() => refreshData())} commissionRate={commissionRate} selfCommissionRate={selfCommissionRate} onUpdateMasterCommissions={handleUpdateMasterCommissions} onLogOnboard={() => setIsCreateModalOpen(true)} appointments={allAppointments} activeCycle={activeCycle} activityLogs={activityLogs} onApplyIncentive={i => supabase.from('incentives').insert({...i, id: generateId(), created_at: new Date().toISOString()}).then(() => refreshData())} allUsers={allUsers} lifetimeTeamEarnings={displayEarnings.lifetime} incentiveRules={allIncentiveRules} onDeleteIncentiveRule={id => supabase.from('incentive_rules').delete().eq('id', id).then(() => refreshData())} allIncentives={allIncentives} /> : currentView === 'user-analytics' ? <div className="p-6 max-w-6xl mx-auto"><UserAnalytics appointments={allAppointments.filter(a => a.userId === user.id)} allAppointments={allAppointments} allIncentives={allIncentives} earnings={displayEarnings} activeCycle={activeCycle} payCycles={payCycles} currentUserName={user.name} currentUser={user} /></div> : currentView === 'calendar' ? <div className="p-6 max-w-6xl mx-auto"><CalendarView appointments={isAdmin ? allAppointments : allAppointments.filter(a => a.userId === user.id)} onEdit={a => {setEditingAppt(a); setIsModalOpen(true);}} onView={a => {setViewingAppt(a); setIsBusinessCardOpen(true);}} userRole={user.role} users={allUsers} activeCycle={activeCycle} /></div> : currentView === 'onboarded' ? <div className="p-6 max-w-6xl mx-auto"><OnboardedView appointments={isAdmin ? allAppointments : allAppointments.filter(a => a.userId === user.id)} searchQuery={searchQuery} onEdit={a => {setEditingAppt(a); setIsModalOpen(true);}} onView={a => {setViewingAppt(a); setIsBusinessCardOpen(true);}} onDelete={id => setDeleteConfirmation({ isOpen: true, id })} users={allUsers} payCycles={payCycles} userRole={user.role} currentWindow={displayEarnings.current} /></div> : currentView === 'earnings-full' ? <div className="p-6 max-w-6xl mx-auto"><EarningsFullView history={displayEarnings.history} currentWindow={displayEarnings.current} appointments={isAdmin ? allAppointments : allAppointments.filter(a => a.userId === user.id)} users={allUsers} userRole={user.role} currentUserName={user.name} onDismissCycle={id => supabase.from('users').update({ dismissed_cycle_ids: [...(user.dismissedCycleIds || []), id] }).eq('id', user.id).then(() => refreshData())} incentives={allIncentives} /></div> : (
            <div className="relative p-6 min-h-full">
                <div className="sticky top-0 z-20 flex justify-end mb-6 pointer-events-none">
                    <button onClick={() => isAdmin ? setIsCreateModalOpen(true) : setIsModalOpen(true)} className="pointer-events-auto flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl shadow-2xl transition-all active:scale-95 text-sm font-black uppercase tracking-widest animate-in slide-in-from-right-4 duration-500 shadow-indigo-200 dark:shadow-none">{isAdmin ? <IconSparkles className="w-5 h-5" /> : <IconPlus className="w-5 h-5" />}{isAdmin ? 'Log Onboarded Partner' : 'New Appointment'}</button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
                    {[AppointmentStage.PENDING, AppointmentStage.RESCHEDULED, AppointmentStage.TRANSFERRED, AppointmentStage.ONBOARDED].map(stage => {
                        // SEARCH LOGIC: Filter by stage, then cycle (for onboarded), then user role, then universal search query.
                        ...
                        const items = (isAdmin ? allAppointments : allAppointments.filter(a => a.userId === user.id)).filter(a => {
                            ...
                            // 1. Cycle Filter for Dashboard 'Onboarded' Column
                            ...
                            if (searchQuery) {
                                const matches = 
                                    ...
                                    (a.aeName && a.aeName.toLowerCase().includes(searchLower)) ||
                                    formatDate(a.scheduledAt).toLowerCase().includes(searchLower);
                        ...
                        if (items.length === 0 && stage !== AppointmentStage.ONBOARDED) return null;
                        ...
                        return (
                            <div key={stage} className="flex flex-col gap-4 animate-in fade-in" onDragOver={e => e.preventDefault()} onDrop={() => draggedId && handleMoveStage(draggedId, stage)}>
                                <div className="flex justify-between px-1">
                                    <h3 className="font-bold text-slate-500 uppercase text-[10px] tracking-widest">
                                        {stage === AppointmentStage.ONBOARDED ? 'Cycle Onboarded' : STAGE_LABELS[stage]}
                                    </h3>
                                    <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 px-2 py-0.5 rounded-full text-[10px] font-bold">{items.length}</span>
                                </div>
                                ...
                                {items.length === 0 && stage === AppointmentStage.ONBOARDED ? (
                                    <div className="p-8 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl opacity-50">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">No Cycle Wins</p>
                                    </div>
                                ) : (
                                    <CardStack<Appointment> items={sortedItems} renderItem={appt => (<div onDragStart={() => setDraggedId(appt.id)} onDragEnd={() => { setDraggedId(null); setIsOverDeleteZone(false); }} draggable><AppointmentCard appointment={appt} onMoveStage={handleMoveStage} onEdit={(a, res) => {setEditingAppt(a); setIsRescheduling(!!res); setIsModalOpen(true);}} onView={a => {setViewingAppt(a); setIsBusinessCardOpen(true);}} onDelete={id => setDeleteConfirmation({ isOpen: true, id })} agentName={isAdmin ? allUsers.find(u => u.id === appt.userId)?.name : undefined} preferredDialer={user.preferredDialer} /></div>)} />
                                )}
                            </div>
                        );
                    })}
                ...
                {/* DELETE ZONE - Revealed only during drag */}
                {draggedId && (
                  <div 
                    onDragOver={e => { e.preventDefault(); setIsOverDeleteZone(true); }}
                    onDragLeave={() => setIsOverDeleteZone(false)}
                    onDrop={() => {
                        setDeleteConfirmation({ isOpen: true, id: draggedId });
                        setDraggedId(null);
                        setIsOverDeleteZone(false);
                    }}
                    className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] transition-all duration-300 pointer-events-auto ${isOverDeleteZone ? 'scale-110' : 'scale-100 opacity-60'}`}
                  >
                     <div className={`px-8 py-5 rounded-[2rem] flex items-center gap-3 shadow-2xl border-2 transition-all duration-300 ${isOverDeleteZone ? 'bg-rose-600 text-white border-rose-400' : 'bg-white dark:bg-slate-900 text-rose-500 border-rose-100 dark:border-rose-900/50'}`}>
                        <IconTrash className={`w-6 h-6 ${isOverDeleteZone ? 'animate-bounce' : ''}`} />
                        <span className="font-black text-xs uppercase tracking-[0.2em]">Drop to Remove</span>
                     </div>
                  </div>
                )}
            </div>
      ...
      <CreateModal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} onSubmit={handleSaveAppointment} isAdminMode={isAdmin} currentUserName={user.name} agentOptions={allUsers.filter(u => u.role !== 'admin')} commissionRate={commissionRate} selfCommissionRate={selfCommissionRate} />
      <AppointmentModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleSaveAppointment} onDelete={id => setDeleteConfirmation({ isOpen: true, id })} initialData={editingAppt} isRescheduling={isRescheduling} agentName={user.name} commissionRate={commissionRate} selfCommissionRate={selfCommissionRate} />
      <BusinessCardModal isOpen={isBusinessCardOpen} onClose={() => setIsBusinessCardOpen(false)} appointment={viewingAppt} onEdit={(a, res) => {setEditingAppt(a); setIsRescheduling(!!res); setIsModalOpen(true);}} onDelete={id => setDeleteConfirmation({ isOpen: true, id })} onMoveStage={handleMoveStage} onSaveNotes={(id, notes) => supabase.from('appointments').update({ notes }).eq('id', id).then(() => refreshData())} />
      ...
      <EarningsPanel isOpen={isEarningsPanelOpen} onClose={() => setIsEarningsPanelOpen(false)} onViewAll={() => { setIsEarningsPanelOpen(false); setCurrentView('earnings-full'); }} currentWindow={displayEarnings.current} history={displayEarnings.history} lifetimeEarnings={displayEarnings.lifetime} teamEarnings={isAdmin ? displayEarnings.lifetime : undefined} teamCurrentPool={isAdmin ? teamCurrentCycleTotal : undefined} isTeamView={isAdmin} />
