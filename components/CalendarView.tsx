...
export const CalendarView: React.FC<CalendarViewProps> = ({ appointments, onEdit, onView, userRole, users = [], activeCycle }) => {
  ...
  const filteredAppointments = useMemo(() => {
    ...
    if (userRole === 'admin') data = data.filter(a => a.stage === AppointmentStage.ONBOARDED || a.stage === AppointmentStage.ACTIVATED);
  ...
  const renderDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // NEW: Activation Detection on this Day
      const dailyActivations = dayAppts.filter(a => a.stage === AppointmentStage.ACTIVATED).length;
      ...
      days.push(
        <div key={day} className="h-28 sm:h-36 border-b border-r border-slate-100 dark:border-slate-800 p-1 sm:p-2 relative group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
            ...
            <div className="flex gap-1 items-center">
              {dailyActivations > 0 && (
                <div className="relative group/ref">
                  <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-sm shadow-emerald-200 dark:shadow-none animate-pulse"></div>
                  <div className="absolute right-0 top-full mt-1 z-50 invisible group-hover/ref:visible opacity-0 group-hover/ref:opacity-100 transition-all duration-200"><div className="bg-emerald-600 text-white text-[9px] rounded-lg py-1 px-2 whitespace-nowrap shadow-xl font-black">{dailyActivations} ACTIVATIONS</div></div>
                </div>
              )}
              {dailyOnboarded > 0 && (
          ...
          <div className="mt-1 space-y-1 overflow-y-auto max-h-[calc(100%-2rem)] no-scrollbar">
            {dayAppts.map(appt => (
              <button key={appt.id} onClick={() => onView?.(appt)} className={`w-full text-left text-[10px] sm:text-xs truncate px-1.5 py-1 rounded border ${appt.stage === AppointmentStage.RESCHEDULED ? 'bg-orange-50 text-orange-700 border-orange-100 dark:bg-orange-900/20 dark:text-orange-300 dark:border-orange-800' : appt.stage === AppointmentStage.ONBOARDED ? 'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-800' : appt.stage === AppointmentStage.ACTIVATED ? 'bg-indigo-50 text-indigo-700 border-indigo-100 dark:bg-indigo-900/20 dark:text-indigo-300 dark:border-indigo-800' : appt.stage === AppointmentStage.PENDING ? 'bg-blue-50 text-blue-700 border-blue-100 dark:bg-blue-900/20 dark:text-blue-300 dark:border-blue-800' : 'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700'}`}>{new Date(appt.scheduledAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} {appt.name}</button>
            ))}
          </div>
        </div>
      );
