...
import { IconX, IconTrash, IconBriefcase, IconCheck, IconSparkles, IconTransfer, IconActivity, IconClock, IconNotes, IconCalendar, IconAlertTriangle } from './Icons';
...
export const AppointmentModal: React.FC<AppointmentModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    aeName: '',
    type: 'appointment' as 'appointment' | 'transfer' | 'activation'
  });
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        let d: Date;
        try {
          d = new Date(initialData.scheduledAt);
  ...
  useEffect(() => {
    ...
    if (cleanPhone.length > 5 || cleanName.length > 3) {
      const match = allAppointments.find(a =>
        (a.phone.replace(/\D/g, '').includes(cleanPhone) && cleanPhone.length > 5) ||
        (a.name.toLowerCase().trim() === cleanName && cleanName.length > 3)
      );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Date formatting error in AppointmentModal:", err);
      scheduledAt = new Date().toISOString();
    }
    ...
    } else if (formData.type === 'activation') {
      finalStage = AppointmentStage.ACTIVATED;
    } else {
      finalAeName = '';
    }
  ...
  const getModalTitle = () => {
    ...
    if (formData.type === 'activation') return 'Log Referral Activation';
              ...
              {matchInfo && (
                <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 animate-in fade-in slide-in-from-top-2">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-700 dark:text-emerald-300">Record Matched</span>
                  </div>
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 leading-relaxed font-medium">
                    Originally <span className="font-bold">{matchInfo.wasSelfOnboard ? 'Self-Onboarded' : `Onboarded (Closed by ${matchInfo.closerName})`}</span> by <span className="font-bold">{matchInfo.agentName}</span> on <span className="font-bold">{new Date(matchInfo.date).toLocaleDateString()}</span>.
                  </p>
                  {matchInfo.earnedAmount > 0 && (
                    <p className="text-[10px] text-emerald-500 font-black uppercase tracking-widest mt-1 italic">
                      ✓ Commission Earned (${(matchInfo.earnedAmount / 100).toFixed(0)})
                    </p>
                  )}
                </div>
              )}
