...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const agentPerformers = useMemo(() => {
    ...
    return allUsers.filter(u => u.role !== 'admin').map(agent => {
      ...
      const crossActivations = performanceStats?.agentStats[agent.id]?.crossActivations || 0;
      const selfOnboardActivations = performanceStats?.agentStats[agent.id]?.selfOnboardActivations || 0;
      const aeAssistedActivations = performanceStats?.agentStats[agent.id]?.aeAssistedActivations || 0;
      return {
        ...
        crossActivations, selfOnboardActivations, aeAssistedActivations
      };
