...
import {
  ...
  IconTrendingUp, IconTransfer, IconActivity, IconChevronDown, IconChevronLeft, IconChevronRight, IconFilter, IconChartPie, IconAlertCircle, IconTimer, IconStar
} from './Icons';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const exportFilteredLedger = () => {
    ...
    const rows = filteredData.onboarded.map(a => {
      ...
      const refPayout = (a.referralCount || 0) * referralRate;
      ...
      return [
        ...
        ((refPayout + stdPayout) / 100).toFixed(2),
        (a.notes || "").replace(/,/g, ";")
      ].join(",");
    });
    const blob = new Blob([[headers.join(","), ...rows].join("\n")], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `ChiCayo_Full_Export_${new Date().toISOString().split('T')[0]}.csv`; a.click();
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-8 no-scrollbar">
        ...
        {activeTab === 'referral-wins' && <ReferralWinsTab appointments={appointments} incentives={allIncentives} users={allUsers} payCycles={payCycles} referralRate={referralRate} currentUser={allUsers.find(u => u.role === 'admin')} />}
